...
fn log(gctx: &GlobalContext, packet: &str, dep: &str, ver: &str) {
    gctx.shell()
        .status(
            "Unifying dependency",
            format!("Unified {dep} @ {ver} of {packet} -> workspace"),
        )
...
fn replace_version_by_workspace<T: TableLike + SortableTableKeysBy + std::fmt::Debug>(
    ...
    let Some(version) = tablelike.remove("version") else {
        return;
    };
    ...
    P: Fn(&Package) -> bool,
{
    ...
    let root_ws_content = std::fs::read_to_string(&manifest_path).context(format!(
    ...
    edit_each(
        members_deep(gctx, ws).iter().filter(|p| predicate(p)),
        |p, doc| {
            let per_table = |deps: &mut Item| {
                let Some(deps) = deps.as_table_mut() else {
                    return Ok(());
                };
